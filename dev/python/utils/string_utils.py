...
def repeat_string_function(
    _function,
    _times: int,
    _args: Iterable[Iterable] = [],
    _separator: str = "\n",
    _end: str = "",
) -> str:
...
def substitute_quotes_whitespace(_string: str, _substitution: str = "!/~/#/") -> str:
    ...
    for match in re.finditer(r'".*?"|\'.*?\'|`.*?`|@media.*?{|calc\(.*?\)', _string):
        if " " in match.group(0):
            ...
            result = (
                result
                + _string[last_index:index]
                + match.group(0).replace(" ", _substitution)
            )
...
def remove_whitespace(_string: str, _separator: str = " ") -> str:
    ...
    result = _separator.join(
        modified_string.replace("\n", _separator)
        .replace("\r", _separator)
        .replace("\t", _separator)
        .replace("\t", _separator)
        .replace("\v", _separator)
        .replace("\f", _separator)
        .split()
    )
    if "!/~/#/" in result:
...
def remove_symbol_spaces(_string: str) -> str:
    ...
    for match in re.finditer(r" *[^\w ] *", modified_string):
        ...
        result = (
            result
            + modified_string[last_index:index]
            + remove_whitespace(match.group(0), "")
        )
        last_index = match.end()
