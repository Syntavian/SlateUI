...
def debug(_func):
    ...
    def wrapper_debug(*args, **kwargs):
        ...
        args_repr = [
            f"{indent(level + 1)}{arg_spec[0][index]} = {colour(format_value(argument), COLOURS['CYAN'])}" for index, argument in enumerate(args)]
        kwargs_repr = [
            f"{indent(level + 1)}{key} = {colour(format_value(argument), COLOURS['CYAN'])}" for key, argument in kwargs.items()]
        signature = ",\n".join(args_repr + kwargs_repr)
        output = f"{repeat_string_function(indent, 2, [[level], (level, 1)])}{colour('>', COLOURS['GREEN'])} {colour(f'{_func.__name__}', COLOURS['CYAN'])} called" + \
