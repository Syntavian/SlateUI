...
def build_css(
    ...
    for input_css_file_line in input_css_file.readlines():
        ...
        if len(line) and line != "\n":
            ...
            if len(matches) > 0 and any(
                (m[0] in _style_selectors or m[1] in _style_selectors) for m in matches
            ):
        ...
        if "@" in input_css_file_line:
            ...
            if "keyframes" in input_css_file_line:
                ...
                output_css_file_text = output_css_file_text + remove_whitespace(
                    input_css_file_line
                )
        if "{" in input_css_file_line:
            block_level += 1
        if is_in_media_block and is_in_required_block:
            media_blocks[current_media_block] = media_blocks[
                current_media_block
            ] + remove_whitespace(input_css_file_line)
        elif is_in_keyframes_block:
            keyframes_blocks[current_keyframes_block] = keyframes_blocks[
                current_keyframes_block
            ] + remove_whitespace(input_css_file_line)
        ...
        if "}" in input_css_file_line:
            block_level -= 1
            if (
                block_level < active_block_level
                and not is_in_keyframes_block
                and not is_in_media_block
            ):
                output_css_file_text = output_css_file_text + remove_whitespace(
                    input_css_file_line
                )
